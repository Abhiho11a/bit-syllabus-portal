...
// Clean dashboard — sidebar nav + clickable stat cards + recent pending list
// Other pages (Pending, SyllabusForm, Submitted) are separate files
...
// ── Mock stats — replace with: api.get('/api/faculty/stats') ─────
const MOCK_STATS = {
  pending:   3,
  submitted: 5,
  approved:  4,
...
// ── Mock recent pending — replace with API ───────────────────────
const MOCK_RECENT = [
  { id:"a1", subject_code:"BCS300", subject_name:"Mathematics III",   sem:3, assigned_by:"Prof. Anitha Rao" },
  { id:"a2", subject_code:"BCS303", subject_name:"Operating Systems", sem:3, assigned_by:"Prof. Anitha Rao" },
  { id:"a3", subject_code:"CS601",  subject_name:"Machine Learning",  sem:6, assigned_by:"Prof. Anitha Rao" },
];
...
const user = JSON.parse(localStorage.getItem("user"))
  ...
  const total   = MOCK_STATS.pending + MOCK_STATS.submitted;
  const donePct = total ? Math.round((MOCK_STATS.submitted / total) * 100) : 0;
  ...
  const CARDS = [
    {
      label:"Pending Tasks",  value: MOCK_STATS.pending,
      icon: Clock,            color:"#f59e0b", bg:"#fffbeb", border:"#fcd34d",
      desc:"Subjects assigned by BOS",    path:"/faculty/pending",
    },
    {
      label:"Submitted",      value: MOCK_STATS.submitted,
      icon: FileText,         color:"#2563eb", bg:"#eff6ff", border:"#bae6fd",
      desc:"Syllabi submitted so far",    path:"/faculty/submitted",
    },
    {
      label:"Approved",       value: MOCK_STATS.approved,
      icon: CheckCircle,      color:"#059669", bg:"#ecfdf5", border:"#6ee7b7",
      desc:"Approved by coordinator",     path:"/faculty/submitted",
    },
    {
      label:"Needs Revision", value: MOCK_STATS.rejected,
      icon: AlertCircle,      color:"#dc2626", bg:"#fef2f2", border:"#fca5a5",
      desc:"Sent back for correction",    path:"/faculty/submitted",
    },
  ];
          ...
          {NAV_LINKS.map(({ label, path, icon: Icon }) => {
            ...
            return (
              ...
              >
                ...
                {label === "Pending Tasks" && MOCK_STATS.pending > 0 && (
        ...
        <div className="px-4 py-4 border-t border-white/10">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl
                       text-sm font-semibold text-red-400 hover:bg-red-500/10
        ...
        <main className="flex-1 p-5 md:p-8">
          ...
          <div className="rounded-2xl p-6 mb-6 relative overflow-hidden"
            ...
            <p className="text-blue-200 text-sm mb-5">
              ...
              {user?.subject_code && (
                <> · <span className="font-mono text-blue-300">{user.subject_code}</span></>
              )}
            </p>
            ...
            <div className="max-w-xs mb-5">
              <div className="flex justify-between text-xs text-blue-300 mb-1.5">
                ...
                <span className="font-bold text-white">{donePct}%</span>
              </div>
              ...
              <p className="text-blue-300 text-xs mt-1.5">
                {MOCK_STATS.submitted} of {total} syllabi submitted
              </p>
            </div>
