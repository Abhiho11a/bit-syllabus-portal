...
import headerFull from "../assets/header_img.png";
...
function handleBarcodeUpload(e) {
  ...
  reader.onload = () => {
    localStorage.setItem("barcodeImg", reader.result); // base64
  };
...
async function generateFooterImage(pageWidthPt, pageNum) {
  ...
  div.innerHTML = `
    ...
    <div style="text-align:center;font-size:9px;color:#000;margin-bottom:5px;">
...
async function mergePDFs(pdfFiles) {
  ...
  // 🔥 only load barcode if dean
  if (isDean) {
    const barcodeBytes = await fetch(barcodeImg).then(res => res.arrayBuffer());
    barcodeImage = await mergedPdf.embedJpg(barcodeBytes);
  }
